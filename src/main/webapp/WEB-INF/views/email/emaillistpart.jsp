		              ...
		              <div class="card-title d-flex" style="padding-top:10px;margin-bottom:0px;padding-bottom:0px">
		              		 <div class=" my-auto" style="text-align:left">
			                    ...
			                    <script>
			                    	...
			                    	//중요한 메일인지 확인한 후 쓰레기통 or 영구삭제 처리
									...
									//검색을 눌렀을 때 해당하는 키워드로 페이지 이동하게 함
						...
						<table class="table table-hover">
							<thead>
								<tr style="border-bottom: 2px solid #004389;">
									<th scope="col"><input name="selectall"
										class="form-check-input" onclick='selectAll(this)' type="checkbox"></input></th>
									<th scope="col">이름</th>
									<th scope="col">제목</th>
									<th scope="col">날짜</th>
								</tr>
							...
							<tbody>
								<c:if test="${not empty emailList}">
									<c:forEach var="emailList" items="${emailList}" varStatus="status">
										<tr>
											<c:if test="${type eq 'receive'}">
												<td><input name="selectone" class="form-check-input"
													value="${emailList.receiveEmailId}" onclick='checkSelectAll()'
													type="checkbox"></td>
											</c:if>
											<c:if
												test="${type eq 'trash'and emailList.sentId eq loginEmployee.empId}">
												<td><input name="selectone" class="form-check-input"
													value="${emailList.sendEmailId}" onclick='checkSelectAll()'
													type="checkbox"></td>
											</c:if>
											<c:if
												test="${type eq 'trash'and emailList.receiveId eq loginEmployee.empId}">
												<td><input name="selectone" class="form-check-input"
													value="${emailList.receiveEmailId}" onclick='checkSelectAll()'
													type="checkbox"></td>
											</c:if>
											<c:if test="${type eq 'temp'}">
												<td><input name="selectone" class="form-check-input"
													value="${emailList.tempEmailId}" onclick='checkSelectAll()'
													type="checkbox"></td>
											</c:if>
											<!-- 임시저장함이 아니고, 내가 보낸 사람일때 받은 사람 출력 -->
											<c:if
												test="${loginEmployee.empId eq emailList.sentId and type ne 'temp' and type ne 'trash'}">
												<td>${emailList.receiveName}</td>
											</c:if>
											<!-- 내가 받은 사람일 때 보낸 사람 출력 -->
											<c:if
												test="${loginEmployee.empId eq emailList.receiveId and type ne 'trash'}">
											...
											<c:if test="${type eq 'receive'}">
												<td><c:if test="${emailList.important}">
														<i style="color: red" class="bi bi-exclamation-circle"></i>[중요]</c:if>
													<a
													href="${pageContext.request.contextPath}/email/readReceiveEmail?receiveEmailId=${emailList.receiveEmailId}">
														${emailList.title} </a></td>
											</c:if>
											<!-- 쓰레기통이고 보낸 사람이 로그인아이디일때 -->
											<c:if
												test="${type eq 'trash'and emailList.sentId eq loginEmployee.empId}">
												<td><c:if test="${emailList.important}">
														<i style="color: red" class="bi bi-exclamation-circle"></i>[중요]</c:if>
													<a
													href="${pageContext.request.contextPath}/email/readSendEmail?sendEmailId=${emailList.sendEmailId}">${emailList.title}</a>
												</td>
											</c:if>
											<!-- 쓰레기통이고 받는 사람이 로그인아이디일때 -->
											<c:if
												test="${type eq 'trash' and emailList.receiveId eq loginEmployee.empId}">
												<td><c:if test="${emailList.important}">
														<i style="color: red" class="bi bi-exclamation-circle"></i>[중요]</c:if>
													<a
													href="${pageContext.request.contextPath}/email/readReceiveEmail?receiveEmailId=${emailList.receiveEmailId}">${emailList.title}</a>
												</td>
											</c:if>
											<!-- 임시보관함일때 -->
											<c:if test="${type eq 'temp'}">
												<td><c:if test="${emailList.important}">
														<i style="color: red" class="bi bi-exclamation-circle"></i>[중요]</c:if>
													<a
													href="${pageContext.request.contextPath}/email/writeTempEmail?tempEmailId=${emailList.tempEmailId}">${emailList.title}</a>
												</td>
											</c:if>
											<!-- 받는 메일함일때 -->
											...
											<c:if
												test="${not empty emailList.strashDate and loginEmployee.empId eq emailList.sentId}">
												<td>${emailList.strashDate}</td>
											</c:if>
											<c:if
												test="${not empty emailList.rtrashDate and loginEmployee.empId eq emailList.receiveId}">
							...
							<div class="d-flex justify-content-center"
								...
								<nav aria-label="Page navigation example">
									<ul class="pagination">
										<li class="page-item">
											<a class="page-link"
												href="${pageContext.request.contextPath}/email/search?keyword=${keyword}&type=${type}&pageNo=1"
										...
										<c:if test="${pager.groupNo>1}">
											<li class="page-item">
												<a class="page-link"
													href="${pageContext.request.contextPath}/email/search?keyword=${keyword}&type=${type}&pageNo=${pager.startPageNo-1}"
										...
										<c:forEach var="i" begin="${pager.startPageNo}"
											...
											<c:if test="${pager.pageNo != i}">
												<li class="page-item"><a class="page-link"
													href="${pageContext.request.contextPath}/email/search?keyword=${keyword}&type=${type}&pageNo=${i}">${i}</a></li>
											</c:if>
											<c:if test="${pager.pageNo == i}">
										...
										<c:if test="${pager.groupNo<pager.totalGroupNo}">
											<li class="page-item">
												<a class="page-link"
													href="${pageContext.request.contextPath}/email/search?keyword=${keyword}&type=${type}&pageNo=${pager.endPageNo+1}"
										...
										<li class="page-item">
											<a class="page-link"
												href="${pageContext.request.contextPath}/email/search?keyword=${keyword}&type=${type}&pageNo=${pager.totalPageNo}"
